...
def enhancedPacmanFeatures(state, action):
    ...
    """
    features = util.Counter()
    succState = state.generateSuccessor(0, action)
    pacmanPos = succState.getPacmanPosition()
    ghostPositions = succState.getGhostPositions()
    ghostStates = succState.getGhostStates()
    foodList = succState.getFood().asList()

    if foodList:
        nearestFood = min([util.manhattanDistance(pacmanPos, foodPos) for foodPos in foodList])
        features['nearestFood'] = float(nearestFood)

    if ghostPositions:
        nearestGhost = min([util.manhattanDistance(pacmanPos, ghostPos) for ghostPos in ghostPositions])
        features['nearestGhost'] = float(nearestGhost)

    for ghostState in ghostStates:
        if ghostState.scaredTimer > 0:
            nearestScaredGhost = min([util.manhattanDistance(pacmanPos, ghostState.getPosition())])
            features['nearestScaredGhost'] = float(nearestScaredGhost)
            break

    features['numGhosts'] = len(ghostPositions)
    features['numFood'] = len(foodList)
    features['score'] = succState.getScore()
    return features
