...
# Converted to Python 3 by Daphne KH
...
def readDigitData(trainingSize=100, testSize=100):
    ...
    featureFunction = lambda x: dataClassifier.basicFeatureExtractorDigit(x)
...
def readDestructData(trainingSize=100, testSize=100):
    rootdata = 'pacmandata'
    rawTrainingData, trainingLabels = samples.loadPacmanData(rootdata + '/destruct_training.pkl', trainingSize)
    rawValidationData, validationLabels = samples.loadPacmanData(rootdata + '/destruct_validation.pkl', testSize)
    rawTestData, testLabels = samples.loadPacmanData(rootdata + '/destruct_test.pkl', testSize)
    trainingData = []
    validationData = []
    testData = []
    return (trainingData, trainingLabels, validationData, validationLabels, rawTrainingData, rawValidationData, testData, testLabels, rawTestData)
...
destructData = readDestructData(1000)
...
DATASETS = {
    ...
    "destructData": lambda: destructData,
...
DATASETS_LEGAL_LABELS = {
    ...
    "destructData": ["EAST", 'WEST', 'NORTH', 'SOUTH', 'STOP'],
...
class MultipleChoiceTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        encryptedSolution = sha1(studentSolution.strip().encode('utf-8').lower()).hexdigest()
